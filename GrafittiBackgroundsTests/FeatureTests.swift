...
class AppCoordintorTests: XCTestCase {
    ...
    func testPreferencesOpens() {
        ...
        preferencesService.actions.setReturnValue(Result<Preferences>.success(Preferences()),
                                                  for: MockPreferencesService.load2.name)
        let preferencesController = PreferencesController(windowController: windowController, preferencesService: preferencesService)
    ...
    func testSystemPreferencesOpens() {
        ...
        XCTAssertEqual(url as? URL, SystemPreference.desktopScreenEffects.url)
    }
